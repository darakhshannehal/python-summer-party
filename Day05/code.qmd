...
# Load Data
pre_sale_data = pd.read_csv('day05_pre_sale_data.csv')
...
pre_sale_data.dropna(subset=['customer_id', 'region', 'pre_order_quantity'], inplace=True)

# Fill less critical missing values
pre_sale_data['demographic_group'].fillna('Unknown', inplace=True)
pre_sale_data['pre_order_date'].fillna('Unknown', inplace=True)  # Or use a placeholder date

# Remove duplicate records
duplicate_count = pre_sale_data.duplicated().sum()
print(f"Number of duplicate records: {duplicate_count}")
pre_sale_data.drop_duplicates(inplace=True)
...
pre_sale_data['month'] = pd.to_datetime(
    pre_sale_data['pre_order_date'], errors='coerce'
).dt.to_period('M')

# Group by region, demographic group, and month
grouped_df = pre_sale_data.groupby(
...
monthly_data = pre_sale_data.copy()
